...
%token TYPE CLASS TOINT TOFLOAT TOSTR TOLITERAL TODBREF TOSYM TOERR VALID
...
%token VERSION RANDOM TIME LOCALTIME MTIME TIMESTAMP STRFTIME
%token CTIME MIN MAX ABS GET_DBREF METHOD_FLAGS METHOD_STATE
%token SET_METHOD_FLAGS SET_METHOD_STATE
%token METHOD THIS DEFINER SENDER CALLER TASK_ID TICKS_LEFT
%token ERROR_FUNC TRACEBACK ERROR_STR ERROR_ARG THROW RETHROW
%token ECHO_FUNC ECHO_FILE STAT_FILE READ_FILE CLOSE_CONNECTION
%token ADD_PARAMETER PARAMETERS DEL_PARAMETER SET_VAR GET_VAR CLEAR_VAR COMPILE
%token METHODS FIND_METHOD FIND_NEXT_METHOD LIST_METHOD DEL_METHOD PARENTS
%token CHILDREN ANCESTORS HAS_ANCESTOR SIZE CREATE CHPARENTS DESTROY LOG
%token REASSIGN_CONNECTION BACKUP BINARY_DUMP TEXT_DUMP
%token EXECUTE SHUTDOWN BIND_PORT UNBIND_PORT OPEN_CONNECTION
%token SET_HEARTBEAT DATA ADD_OBJNAME DEL_OBJNAME NEXT_DBREF TICK HOSTNAME IP
%token RESUME SUSPEND TASKS CANCEL PAUSE CALLERS LOAD STATUS
%token BIND_FUNCTION UNBIND_FUNCTION TOKENIZE_CML BUF_TO_VEIL_PACKETS
%token BUF_FROM_VEIL_PACKETS ATOMIC NON_ATOMIC METHOD_ARGS
