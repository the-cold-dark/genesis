...
%token F_METHODOP F_THIS F_DEFINER F_SENDER F_CALLER F_TASK_ID
%token F_TICKS_LEFT F_ERROR_FUNC F_TRACEBACK F_ERROR_STR
...
%token F_DEL_VAR F_SET_VAR F_GET_VAR F_CLEAR_VAR F_ADD_METHOD
%token F_RENAME_METHOD F_METHODS F_FIND_METHOD F_FIND_NEXT_METHOD
%token F_COMPILE F_GET_METHOD F_DECOMPILE F_DEL_METHOD F_PARENTS
%token F_CHILDREN F_ANCESTORS F_HAS_ANCESTOR F_DESCENDANTS F_SIZE
%token F_CREATE F_CHPARENTS F_DESTROY F_LOG F_REASSIGN_CONNECTION
...
%token F_TASKS F_CANCEL F_PAUSE F_REFRESH F_STACK F_STATUS
%token F_BIND_FUNCTION F_UNBIND_FUNCTION F_ATOMIC
%token F_METHOD_INFO F_ENCODE F_DECODE
